...
function Sphere(props) {
    ...
    const uniforms = useRef(THREE.UniformsUtils.merge([
...
const material = useRef(new THREE.ShaderMaterial({
  ...
  useFrame((state, delta) => {
    material.current.uniforms.time.value = 0.00001 * (Date.now() - start.current);
    mesh.current.rotation.z += 0.001
  })
  

  const { radius, heightSegments, widthSegments } = useControls('Sphere', { widthSegments: {value: 64}, heightSegments: {value: 64}, radius:{ value: 10} })
